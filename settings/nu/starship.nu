...
let-env PROMPT_MULTILINE_INDICATOR = (^'\\?\E:\Alacritty\vendor\starship.exe' prompt --continuation)
...
let-env PROMPT_COMMAND = {
    ...
    ^'\\?\E:\Alacritty\vendor\starship.exe' prompt $"--cmd-duration=($env.CMD_DURATION_MS)" $"--status=($env.LAST_EXIT_CODE)" $"--terminal-width=($width)"
}
